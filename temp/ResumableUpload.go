...
func ResumableUploadInitHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Session upload initialized for file: %s, size: %v, session ID: %s, sha256: %s", req.FilePath, req.Size, sessionID, req.Sha256)
...
func ResumableUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Flag finish: %v, end: %v, total size: %v", flagFinish, end, session.CountByte)
	...
	if flagFinish {
		// Sync file to ensure all data is written to disk before checksum verification
